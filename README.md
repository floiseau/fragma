...
`fragma` is finite element solver to simulate crack propogation in anisotropic media using the phase-field approach to fracture.
This solver uses `fenicsx`.

## Installation
To run `fragma`, different python modules must be installed first.
To install those modules inside an environment, one can apply the following commands.
```shell
$ conda create -n fenicsx-env
$ conda activate fenicsx-env
$ conda install -c conda-forge fenics-dolfinx mpich pyvista python-gmsh mpi4py petsc4py
```
In practice, I used `micromamba` install of `conda` to manage the environment.


## Usage
In order to run, `fragma` need a GMSH file and the parameter file (`parameters.toml`).
The content of the parameter file is described below and examples are given in the `examples` directory.

Once both files are provided, differents steps are necessary to run it.
...
*It is also possible to run* `fragma` *directly in the environment using* `conda run -n fenicsx-env python path/to/repo/fragma/main.py` *(or the same comment with*`micromamba`*). Also note that the examples contain a* `run.sh` *file which generate the mesh and run* `fragma`*.*
...
*Remark: Note that if you use `conda`, you will need to update the `run.sh` files in the example by replacing `micromamba` with `conda`.*
