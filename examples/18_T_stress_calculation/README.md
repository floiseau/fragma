...
## Description
This example is dedicated to the post-process of the $T$-stress.
...
To obtain the $T$-stress under a loading, the `postprocess` section of the paramter file must be filled as shown below
```toml
crack_tip = [0.05, 0]
R_int = ...
R_ext = ...
...
T = - \sigma \cos(2*\alpha) 
```
where $\sigma$ is the applied stress along $y$-axis, $\alpha$ is the initial crack angle.
...
The script `display_results.py` shows a scatter plot with the evolution of the $T$-stress has a function of both parameters.

## Expected results

In this exemple, the `probes.csv` files contains the T-stress in the column `T_stress`.
Running the example gives a value around  $-1$ Pa which is close the analytical solution.
