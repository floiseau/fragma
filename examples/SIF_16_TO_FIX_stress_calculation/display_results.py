...
beta = np.pi / 2 - alphas
K1_ana = np.sin(beta) ** 2  # NOTE: Error in Eq 23 of Smith at al. (2001), see Eq 19
K2_ana = np.cos(beta) * np.sin(beta)
T_ana = np.cos(2 * beta)
Beq_ana = np.cos(2 * beta) / np.sin(beta)
# beta = np.pi/2 - alphas
...
plt.scatter(alphas_deg, K1_res, c="#f1443a", label="FEM - fragma")
...
plt.scatter(alphas_deg, K2_res, c="#f1443a", label="FEM - fragma")
plt.xlabel("alpha $\\alpha$ (°)")
...
plt.scatter(alphas_deg, T_res, c="#f1443a", label="FEM - fragma")
plt.xlabel("alpha $\\alpha$ (°)")
...
plt.scatter(alphas_deg[:-1], Beq_res[:-1], c="#f1443a", label="FEM - fragma")
plt.xlabel("alpha $\\alpha$ (°)")
